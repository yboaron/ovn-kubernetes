/*


Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/
// Code generated by applyconfiguration-gen. DO NOT EDIT.

package v1alpha1

// NetworkInfoApplyConfiguration represents a declarative configuration of the NetworkInfo type for use
// with apply.
type NetworkInfoApplyConfiguration struct {
	Name      *string                     `json:"name,omitempty"`
	CIDR      []string                    `json:"cidr,omitempty"`
	Topology  *string                     `json:"topology,omitempty"`
	Transport *string                     `json:"transport,omitempty"`
	EVPN      *EVPNInfoApplyConfiguration `json:"evpn,omitempty"`
}

// NetworkInfoApplyConfiguration constructs a declarative configuration of the NetworkInfo type for use with
// apply.
func NetworkInfo() *NetworkInfoApplyConfiguration {
	return &NetworkInfoApplyConfiguration{}
}

// WithName sets the Name field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the Name field is set to the value of the last call.
func (b *NetworkInfoApplyConfiguration) WithName(value string) *NetworkInfoApplyConfiguration {
	b.Name = &value
	return b
}

// WithCIDR adds the given value to the CIDR field in the declarative configuration
// and returns the receiver, so that objects can be build by chaining "With" function invocations.
// If called multiple times, values provided by each call will be appended to the CIDR field.
func (b *NetworkInfoApplyConfiguration) WithCIDR(values ...string) *NetworkInfoApplyConfiguration {
	for i := range values {
		b.CIDR = append(b.CIDR, values[i])
	}
	return b
}

// WithTopology sets the Topology field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the Topology field is set to the value of the last call.
func (b *NetworkInfoApplyConfiguration) WithTopology(value string) *NetworkInfoApplyConfiguration {
	b.Topology = &value
	return b
}

// WithTransport sets the Transport field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the Transport field is set to the value of the last call.
func (b *NetworkInfoApplyConfiguration) WithTransport(value string) *NetworkInfoApplyConfiguration {
	b.Transport = &value
	return b
}

// WithEVPN sets the EVPN field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the EVPN field is set to the value of the last call.
func (b *NetworkInfoApplyConfiguration) WithEVPN(value *EVPNInfoApplyConfiguration) *NetworkInfoApplyConfiguration {
	b.EVPN = value
	return b
}
