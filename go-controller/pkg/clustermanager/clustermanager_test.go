	...
	ginkgo.Context("Managed VTEP + RouteAdvertisements integration", func() {
		...
		ginkgo.It("VTEP controller allocates IPs and RA generates /32 host-route FRRConfigurations", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				// The RA is intentionally NOT pre-seeded. We start with CUDN+VTEP+FRR
				// only, wait for the UDN controller to create the NAD and the network
				// manager to register cluster_udn_blue, and only then create the RA.
				// This guarantees that when the RA controller first reconciles, it can
				// call GetNetwork("cluster_udn_blue") successfully and set Accepted=True
				// without a race against async network registration.
				...
				// Only create the RA now that the NAD exists and the network manager
				// has registered cluster_udn_blue. The RA controller's first reconcile
				// will call GetNetwork and find the network immediately — no race.
