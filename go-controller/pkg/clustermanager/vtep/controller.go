...
func (c *Controller) cleanupManagedVTEPAnnotations(vtepName string) error {
	...
	for _, node := range nodes {
		if err := c.removeVTEPAnnotationEntry(vtepName, node.Name); err != nil {
...
// removeVTEPAnnotationEntry removes a single VTEP entry from the node's
// k8s.ovn.org/vteps annotation using retry-on-conflict.
func (c *Controller) removeVTEPAnnotationEntry(vtepName, nodeName string) error {
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		annotations, err := util.MarshalNodeVTEPs(vteps)
		if err != nil {
			return fmt.Errorf("failed to marshal VTEP annotation: %w", err)
		}
		for k, v := range annotations {
			if v == nil {
				delete(cnode.Annotations, k)
			} else {
				cnode.Annotations[k] = v.(string)
			}
		}
		return c.kube.UpdateNodeStatus(cnode)
	})
}
...
func (c *Controller) allocateAndAnnotateNode(vtepName, nodeName string, allocator *vtepIPAllocator) error {
	...
	// The annotation is a single JSON blob shared between the cluster-manager
	// (managed VTEPs) and ovnkube-node (unmanaged VTEPs). Both sides do
	// read-modify-write, so a concurrent write from the node side between
	// our read and our UpdateNodeStatus will cause a 409 Conflict because
	// the resourceVersion is stale. RetryOnConflict handles this.
	//
	// On retry we re-read from the informer cache (nodeLister), not the API
	// server. If the informer hasn't caught up yet, the stale resourceVersion
	// triggers another 409. DefaultBackoff provides 5 retries with
	// exponential delays (10ms, 20ms, 40ms, …) which gives the informer
	// time to sync. This is the same approach used by
	// NodeAllocator.updateNodeNetworkAnnotationsWithRetry for the
	// k8s.ovn.org/node-subnets annotation that has the same shared-key
	// pattern across multiple network controllers.
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		node, err := c.nodeLister.Get(nodeName)
		if err != nil {
			return fmt.Errorf("failed to get node %s: %w", nodeName, err)
		...
		vteps, err := util.ParseNodeVTEPs(cnode)
		if err != nil {
			if !util.IsAnnotationNotSetError(err) {
				return fmt.Errorf("failed to parse VTEP annotation: %w", err)
			}
			vteps = map[string]util.VTEPNodeAnnotation{}
		}
